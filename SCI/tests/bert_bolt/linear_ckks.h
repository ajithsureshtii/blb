...
public:
	...
	*/
	...
	Linear_ckks();
	Linear_ckks(int party, NetIO *io);
	...
	vector<Ciphertext> w_ln(HE_CKKS* he, vector<Ciphertext> ln, vector<Plaintext> w);
	...
	vector<vector<vector<Ciphertext>>> preprocess_softmax_s1_ct_ct(HE_CKKS* he, const vector<Ciphertext> &matrix, const FCMetadata &data, vector<vector<Plaintext>> &mask);
