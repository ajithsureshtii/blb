...
public:
    ...
    HE_CKKS(int party,
        ...
        if (party == BOB) { // BOB, client
            ...
            // vector<double> pod_matrix(poly_modulus_degree/2, 0ULL);
            // Plaintext tmp;
            // encoder->encode(pod_matrix,pow(2,20), tmp);
            // zero = new Ciphertext;
    ...
    vector<scalar_t> ckks_decode(vector<complex_t> x, int degree);

	vector<scalar_t> ckks_encode(vector<scalar_t> x, int slot);
