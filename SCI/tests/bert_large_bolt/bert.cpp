...
double trunc_comm = 0;
double trunc_round = 0;
double other_nonlinear_comm = 0;
double other_nonlinear_round = 0;
double cts_transfer_comm = 0;
double cts_transfer_round = 0;
double offline_comm = 0;
double offline_round = 0;
double ln_comm = 0;
double ln_round = 0;
double gelu_comm = 0;
double gelu_round = 0;
double softmax_comm = 0;
double softmax_round = 0;
...
Bert::Bert(int party, int port, string address, string model_path, bool prune){
    ...
    offline_comm += this->get_comm();
    offline_round += this->get_round();
}
...
void Bert::he_to_ss_server(HE* he, vector<Ciphertext> in, uint64_t* output, bool ring){
    ...
    he_compute_la += interval(start);
    start = high_resolution_clock::now();
    send_encrypted_vector(io, cts);
    ct_transfer_la += interval(start);
    #ifdef BERT_PERF
    t_total_conversion += interval(t_conversion);
    #endif 
    #ifdef BERT_PERF_XTS
    cts_transfer_comm += get_comm();
    cts_transfer_round += get_round();
    #endif
}
// length是input的总长度
...
vector<double> Bert::run(string input_fname, string mask_fname){
    ...
    double comm_init = n_comm;
    double round_init = n_rounds;
    ...
    if(party == sci::ALICE){
        ...
        cout << "input path: " << input_fname << endl;
        // vector<vector<uint64_t>> input_plain = read_data(input_fname);
        vector<vector<uint64_t>> input_plain(input_dim, vector<uint64_t>(COMMON_DIM, 1));
        // softmax_mask = read_bias(mask_fname, 128);
        softmax_mask = vector<uint64_t>(128, 1);
    ...
    for(int layer_id = 0; layer_id < ATTENTION_LAYERS; ++layer_id){
        {
            ...
            if(party == sci::ALICE){
                ...
                cout << "q_k_v.size: " << q_k_v.size() << endl;
                int qk_offset = qk_size / data.slot_count;
                cout << "qk_offset: " << qk_offset << endl;
                ...
                int qk_cts_len = ceil(qk_size*1.0 / lin.he_8192->poly_modulus_degree);
                cout << "qk_cts_len: " << qk_cts_len << endl;
            ...
            #ifdef XTS
            if(layer_id == 1){
                cout << "end attention linear part" << endl;
                cout << "total comm: " << get_comm() << endl;
                cout << "total round: " << get_round() << endl;
            }
            #endif
            ...
            #ifdef BERT_PERF_XTS
            conversion_la += interval(start);
            conversion_comm += get_comm();
            conversion_round += get_round();
            #endif
            start = high_resolution_clock::now();
            nl.gt_p_sub_part2(
                NL_NTHREADS,
            ...
            #ifdef BERT_PERF_XTS
            trunc_la += interval(start);
            trunc_comm += get_comm();
            trunc_round += get_round();
            #endif
            #ifdef XTS
            if(layer_id == 1){
                cout << "end layernorm+attention" << endl;
                cout << "total comm: " << get_comm() << endl;
                cout << "total round: " << get_round() << endl;
                // exit(0);
            }
            ...
            cout << "ok 982" << endl;
            #ifdef BERT_PERF_XTS
            softmax_la += interval(start);
            softmax_comm += get_comm();
            softmax_round += get_round();
            #endif
            #ifdef BERT_SAVE_RESULTS
            ...
            if(party == sci::ALICE){
                ...
                auto pack_softmax_ct = lin.preprocess_softmax_s1_ct_ct(lin.he_8192, enc_softmax, data);
                ...
                cout << "softmax_v time: " << interval(startt) << endl;
                // exit(0);
                he_compute_la += interval(start);
                ...
                int cts_len = PACKING_NUM * data.image_size * data.filter_w / data.slot_count;
        ...
        // -------------------- Linear #2 -------------------- //
        {
            ...
            #ifdef XTS
            cout << "begin layer norm + linear3 + gelu" << endl;
            get_comm();
            get_round();
            #endif
            start = high_resolution_clock::now();
            ...
            #ifdef BERT_PERF_XTS
            ln_la += interval(start);
            ln_comm += get_comm();
            ln_round += get_round();
            #endif
            #ifdef XTS
            cout << "end layernorm" << endl;
            get_comm();
            get_round();
            #endif
            // wx
            if(party == sci::ALICE){
                vector<Ciphertext> ln = ss_to_he_server(
                    ...
                    NL_ELL);
                start = high_resolution_clock::now();
                vector<Ciphertext> ln_w = lin.w_ln(lin.he_8192_ln, ln);
                he_compute_la += interval(start);
            ...
            #endif 
            #ifdef XTS
            cout << "nonlinear part after layernorm" << endl;
            cout << "comm: " << get_comm() << endl;
        ...
        // -------------------- Linear #3 -------------------- //
        {
            ...
            #endif 
            #ifdef XTS
            cout << "nonlinear part after linear3, and gelu begin" << endl;
            cout << "comm: " << get_comm() << endl;
            cout << "round: " << get_round() << endl;
            // exit(0);
            #endif
            ...
            #ifdef BERT_PERF_XTS
            gelu_la += interval(start);
            gelu_comm += get_comm();
            gelu_round += get_round();
            #endif
            #ifdef XTS
            cout << "gelu end" << endl;
            cout << "comm: " << get_comm() << endl;
            ...
            #endif
            #ifdef XTS
            cout << "end layer norm + linear3 + gelu" << endl;
            cout << "comm:" << get_comm() << ", round:" << get_round() << endl;
            #endif
...

        {
            ...
            #ifdef XTS
            cout << "begin layer norm + attention block" << endl;
            get_comm();
            get_round();
            #endif
            start = high_resolution_clock::now();
            ...
            #ifdef XTS
            cout << "nonlinear after layernorm" << endl;
            cout << "comm:" << get_comm() << ", round:" << get_round() << endl;
            #endif
            #ifdef BERT_PERF
            t_total_repacking += interval(t_repacking_2);
    ...
    total_la = interval(start_init);
    #ifdef BERT_PERF_XTS
    cout << "-----------comm------------" << endl;
    cout << "cts_transfer_comm:" << cts_transfer_comm << endl;
    cout << "conversion_comm:" << conversion_comm << endl;
    cout << "truncate_comm:" << trunc_comm << endl;
    cout << "other_nonlinear_comm:" << other_nonlinear_comm << endl;
    cout << "ln_comm:" << ln_comm << endl;
    cout << "gelu_comm:" << gelu_comm << endl;
    cout << "softmax_comm:" << softmax_comm << endl;
    cout << "total comm:" << n_comm-comm_init << endl;
    cout << "comm should be 0, :" << get_comm() << endl;
    cout << "other comm, should be 0. :" << n_comm-comm_init-cts_transfer_comm-conversion_comm-trunc_comm-other_nonlinear_comm-ln_comm-gelu_comm-softmax_comm << endl;
    cout << "-----------round------------" << endl;
    cout << "cts_transfer_round:" << cts_transfer_round << endl;
    cout << "conversion_round:" << conversion_round << endl;
    cout << "truncate_round:" << trunc_round << endl;
    cout << "other_nonlinear_round:" << other_nonlinear_round << endl;
    cout << "ln_round:" << ln_round << endl;
    cout << "gelu_round:" << gelu_round << endl;
    cout << "softmax_round:" << softmax_round << endl;
    cout << "total round:" << n_rounds-round_init << endl;
    cout << "other round, should be 0. :" << n_rounds-round_init-cts_transfer_round-conversion_round-trunc_round-other_nonlinear_round-ln_round-gelu_round-softmax_round << endl;
    cout << "-----------time-------------" << endl;
    cout << "total_la:" << total_la << endl;
    cout << "ct_transfer_la:" << ct_transfer_la << endl;
    cout << "conversion_la:" << conversion_la << endl;
    cout << "truncate_la:" << trunc_la << endl;
    cout << "he_compute_la:" << he_compute_la << endl;
    cout << "other_nonlinear_la:" << other_nonlinear_la << endl;
    cout << "ln_la:" << ln_la << endl;
    cout << "gelu_la:" << gelu_la << endl;
    cout << "softmax_la:" << softmax_la << endl;
    cout << "other la:" << total_la - ct_transfer_la - conversion_la - trunc_la - other_nonlinear_la - ln_la - gelu_la - softmax_la-he_compute_la << endl;
    #endif
    exit(0);
    ...
    cout << "> [NETWORK]: Communication rounds: " << total_rounds << endl; 
    cout << "> [NETWORK]: Communication overhead: " << total_comm << " bytes" << endl; 
    #endif 
