...
Linear::Linear(int party, sci::NetIO *io, bool prune) {
	this->party = party;
	this->io = io;
	this->he_8192 = new HE(
		party,
...
void Linear::set_HE(
    ...
    this->he = new HE(party, io, poly_modulus_degree, coeff_bit_sizes, plain_mod_bit, bit);
}
...
vector<Ciphertext> Linear::linear_1(
    ...
    // check_cts(Cipher_plain_results);
    cout << "bert_cipher_plain_bsgs OK" << endl;
...
Linear::bert_cross_packing_single_matrix(HE* he, const vector<vector<vector<uint64_t>>> &weights, const FCMetadata &data) {
    vector<pair<vector<vector<Plaintext>>, vector<vector<Plaintext>>>> result(PACKING_NUM/2);
...
void Linear::bert_cipher_plain_bsgs(
    ...
    #pragma omp parallel for num_threads(cts.size() * n1)
    ...
    cout << "temp_result_size: " << temp_result_size << endl;
    ...
    omp_set_nested(1);
    Plaintext w1;
    vector<uint64_t> vec_w1(data.slot_count,1);
    he->encoder->encode(vec_w1, w1);
    #pragma omp parallel for num_threads(PACKING_NUM/2)
    for (int packing_index = 0; packing_index < PACKING_NUM/2; packing_index++) {
        // cout << "packing_index: " << packing_index << endl;
        ...
        for (int l = 0; l < temp_result_size; l++) {
            ...
            // cout << "OK4" << endl;
            he->evaluator->add_plain_inplace(result[l + packing_index * data.image_size * data.filter_w * 2 / data.slot_count], w1);
            he->evaluator->add_plain_inplace(result[l + packing_index * data.image_size * data.filter_w * 2 / data.slot_count + data.image_size * data.filter_w * PACKING_NUM / data.slot_count], w1);
            he->evaluator->add_plain_inplace(result[l + packing_index * data.image_size * data.filter_w * 2 / data.slot_count + data.image_size * data.filter_w * PACKING_NUM*2 / data.slot_count], w1);
        }
    }
    // cout << "OK3" << endl;
}
...
void Linear::bert_cipher_plain_bsgs_2(
    ...
    vector<uint64_t> vec_w1(data.slot_count,1);
    he->encoder->encode(vec_w1, w1);
    #pragma omp parallel for num_threads(cts.size() * n2)
    for (int k = 0; k < cts.size() * n2; k++) {
        int j = k / cts.size();
        int ct_i = k % cts.size();
    ...
    for (int l = 0; l < data.image_size * data.filter_w / data.slot_count; l++) {
        ...
        result[l] = ct;
        he->evaluator->add_plain_inplace(result[l], w1);
    }
...
void Linear::bert_cipher_cipher_cross_packing(
    ...
    for (int packing_index = 0; packing_index < PACKING_NUM/2; packing_index++) {
        ...
        for (int i = 0; i < data.image_size * 2; i++) {
            ...
            he->evaluator->multiply_plain_inplace(rotation_results[i], w1);
        }
        int num_cts_per_res = ceil(data.image_size * data.image_size * 2.0 / data.slot_count); // 1 or 4
...
void Linear::bert_softmax_V(HE* he, vector<Ciphertext> &softmax_s1, vector<Ciphertext> &V,  const FCMetadata &data, vector<Ciphertext> &result) {
    ...
    for (int packing_ind = 0; packing_ind < PACKING_NUM/2; packing_ind++) {
        ...
        for (int ct_ind = 0; ct_ind < num_matrix_per_col; ct_ind++) {
            ...
            for (int i = 0; i < num_diag; i++) {
                ...
                he->evaluator->multiply_plain(temp_mult[0], w1, temp1);
                he->evaluator->multiply_plain(temp_mult[1], w1, temp2);
...
vector<Ciphertext> Linear::w_ln(HE* he, vector<Ciphertext> ln){
    int cts_size = ln.size();
    vector<Ciphertext> result(cts_size);
    Plaintext w1;
    vector<uint64_t> vec_w1(he->poly_modulus_degree,1);
    he->encoder->encode(vec_w1, w1);
    #pragma omp parallel for
...
vector<vector<vector<Ciphertext>>> Linear::preprocess_softmax_s1_ct_ct(HE* he, const vector<Ciphertext> &matrix, const FCMetadata &data) {
