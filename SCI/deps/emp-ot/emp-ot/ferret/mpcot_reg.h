...
public:
	...
	IO **ios;
	emp::block Delta_f2k;
	emp::block *consist_check_chi_alpha = nullptr, *consist_check_VW = nullptr;
	ThreadPool *pool;
	...
	void sender_init(emp::block delta) {
		Delta_f2k = delta;
	}
	...
	// MPFSS F_2k
	void mpcot(emp::block * sparse_vector, OTPre<IO> * ot, emp::block *pre_cot_data) {
		if(party == emp::BOB) consist_check_chi_alpha = new emp::block[item_n];
		consist_check_VW = new emp::block[item_n];
	...
	void exec_parallel_sender(vector<SPCOT_Sender<IO>*> &senders,
			OTPre<IO> *ot, emp::block* sparse_vector) {
	...
	void exec_f2k_sender(SPCOT_Sender<IO> *sender, OTPre<IO> *ot, 
			emp::block *ggm_tree_mem, IO *io, int i) {
	...
	void consistency_check_f2k(emp::block *pre_cot_data, int num) {
		if(this->party == emp::ALICE) {
			...
			this->netio->send_data(dig, 2*sizeof(emp::block));
			this->netio->flush();
		} else {
			emp::block r1, r2, r3;
			vector_self_xor(&r1, this->consist_check_VW, num);
			...
			hash.hash_once(dig, &r1, sizeof(emp::block));
			emp::block recv[2];
			this->netio->recv_data(recv, 2*sizeof(emp::block));
