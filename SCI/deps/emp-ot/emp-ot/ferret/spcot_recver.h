...
public:
	...
	void recv_f2k(OT * ot, IO * io2, int s) {
		...
		io2->recv_data(&secret_sum_f2, sizeof(emp::block));
	}

	// receive the message and reconstruct the tree
	// j: position of the secret, begins from 0
	void compute(emp::block* ggm_tree_mem) {
		this->ggm_tree = ggm_tree_mem;
		ggm_tree_reconstruction(b, m);
		ggm_tree[choice_pos] = emp::zero_block;
		emp::block nodes_sum = emp::zero_block;
		emp::block one = makeBlock(0xFFFFFFFFFFFFFFFFLL,0xFFFFFFFFFFFFFFFELL);
	...
	void ggm_tree_reconstruction(bool *b, emp::block *m) {
		int to_fill_idx = 0;
		TwoKeyPRP prp(emp::zero_block, makeBlock(0, 1));
	...
	void layer_recover(int depth, int lr, int to_fill_idx, emp::block sum, TwoKeyPRP *prp) {
		int layer_start = 0;
		int item_n = 1<<depth;
		emp::block nodes_sum = emp::zero_block;
	...
	void consistency_check_msg_gen(emp::block *chi_alpha, emp::block *W) {
		// X
		emp::block *chi = new emp::block[leave_n];
		emp::Hash hash;
		emp::block digest[2];
		hash.hash_once(digest, &secret_sum_f2, sizeof(emp::block));
